...
namespace EuclidianSpacetime.Rendering
{
    public static class TimeRenderer
    {
        ...
        /// </summary>
        public static IEnumerable<IEnumerable<(int[], ARGB32)>> Render(ISpacetime spacetime, ITimeArrow timeArrow, DimensionInfo dimensionInfo)
        {
            var n = spacetime.N;
            if (n <= 0)
            {
        ...
        {
            var dimensionInfo = SpaceRenderer.GetDimensionInfo(0, linearUnitsPerFrame, region);
